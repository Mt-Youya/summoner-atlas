...
export default async function LoginPage() {
  ...
  return (
    <PageFrame>
      ...
      <Link className="inline-flex border-b border-foreground pb-1 text-sm" href={localizePath("/zh/profile", locale)}>
...
export async function generateMetadata() {
  const locale = await getLocale()
  return {
    title: `${t(locale, "loginBinding")} | Summoner Atlas`,
    alternates: { canonical: canonical(localizePath("/zh/login", locale)) },
  }
}
