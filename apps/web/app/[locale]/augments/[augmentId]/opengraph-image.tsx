...
import { translateAugmentName } from "@summoner-atlas/i18n"

export const alt = "Summoner Atlas"
export const size = { width: 1200, height: 630 }
export const contentType = "image/png"

export default async function OpenGraphImage({ params }: { params: Promise<{ locale: string; augmentId: string }> }) {
  const { locale, augmentId } = await params
  const augment = await getAugment(Number(augmentId))
  const name = augment ? translateAugmentName(augment.name, locale) : "Augment data"
