  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      setAugments(augResults.filter((a) =>
        a.augment.name.toLowerCase().includes(query.toLowerCase()) ||
        a.augment.nameZh.includes(query)
      ))
    }, 150)
      ...
      <DialogContent className="sm:max-w-[560px] p-0 shadow-[var(--glow-high)]">
        ...
        <Command>
              ...
              <CommandGroup heading={t("champions")}>
                {champions.slice(0, 6).map((c) => (
                  <CommandItem key={c.champion.id} onSelect={() => navigate(`/champions/${c.champion.id}`)}>
                    ...
                    <Badge variant="secondary" className="text-[10px]">{c.winRate.toFixed(1)}%</Badge>
                  </CommandItem>
                ))}
              </CommandGroup>
            )}
              ...
              <CommandGroup heading={t("augments")}>
                {augments.slice(0, 6).map((a) => (
                  <CommandItem key={a.augment.id} onSelect={() => navigate(`/augments/${a.augment.id}`)}>
                    ...
                    <Badge variant="secondary" className="text-[10px]">{a.winRate.toFixed(1)}%</Badge>
                  </CommandItem>
                ))}
              </CommandGroup>
            )}
          ...
          <div className="border-t px-3 py-2 text-[11px] text-muted-foreground flex items-center gap-3">
            <span><kbd className="px-1 py-0.5 rounded bg-muted text-[10px] font-mono">↑↓</kbd> {t("sort")}</span>
            <span><kbd className="px-1 py-0.5 rounded bg-muted text-[10px] font-mono">Enter</kbd> Open</span>
